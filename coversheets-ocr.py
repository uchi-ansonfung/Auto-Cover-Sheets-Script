...
print(f"Automatic Exhibit Cover Sheets v0.3 with OCR.  Anson Fung")
...
def add_cover_to_pdf(cover_buffer, original_path, output_path):
    ...
    """
    # Create temporary PDF file for initial output
    with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_file:
        temp_path = temp_file.name  
    
    # Merge cover sheet and original PDF
    ...
    # Run OCRmyPDF to OCR the temporary PDF and save to output path
    try:
        subprocess.run([
            'ocrmypdf',
            '-l', 'eng',  # Specify English for OCR
            '--pdf-renderer', 'hocr',  # Use hOCR for accurate text positioning
            '--output-type', 'pdf' # Disable PDF/A output for smaller file size; lossless compression enabled by default
            '--deskew',  # Correct skewed pages
            temp_path,
            output_path
        ], check=True)
    except subprocess.CalledProcessError as e:
        print(f"OCR failed for {original_path}: {e}")
        os.unlink(temp_path)
        raise
    except FileNotFoundError:
        print("OCRmyPDF is not installed or not found in PATH. Please install it.")
        os.unlink(temp_path)
        raise
